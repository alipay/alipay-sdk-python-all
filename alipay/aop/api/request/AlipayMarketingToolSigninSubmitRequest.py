...
class AlipayMarketingToolSigninSubmitRequest(object):
    ...
    def get_params(self):
        params = dict()
        params[P_METHOD] = 'alipay.marketing.tool.signin.submit'
        params[P_VERSION] = self.version
        if self.biz_model:
            params[P_BIZ_CONTENT] = json.dumps(obj=self.biz_model.to_alipay_dict(), ensure_ascii=False, sort_keys=True, separators=(',', ':'))
        if self.play_id:
            if hasattr(self.play_id, 'to_alipay_dict'):
                params['play_id'] = json.dumps(obj=self.play_id.to_alipay_dict(), ensure_ascii=False, sort_keys=True, separators=(',', ':'))
            else:
