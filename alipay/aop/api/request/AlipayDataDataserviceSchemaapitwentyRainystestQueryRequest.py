...
from alipay.aop.api.domain.RainyComplexTypesTheFifteenOne import RainyComplexTypesTheFifteenOne



class AlipayDataDataserviceSchemaapitwentyRainystestQueryRequest(object):
    ...
    def demo_wink_ref(self, value):
        if isinstance(value, RainyComplexTypesTheFifteenOne):
            self._demo_wink_ref = value
        else:
            self._demo_wink_ref = RainyComplexTypesTheFifteenOne.from_alipay_dict(value)
    ...
    def get_params(self):
        params = dict()
        params[P_METHOD] = 'alipay.data.dataservice.schemaapitwenty.rainystest.query'
        params[P_VERSION] = self.version
        if self.biz_model:
            params[P_BIZ_CONTENT] = json.dumps(obj=self.biz_model.to_alipay_dict(), ensure_ascii=False, sort_keys=True, separators=(',', ':'))
        if self.demo_strong_ref:
            if hasattr(self.demo_strong_ref, 'to_alipay_dict'):
                params['demo_strong_ref'] = json.dumps(obj=self.demo_strong_ref.to_alipay_dict(), ensure_ascii=False, sort_keys=True, separators=(',', ':'))
            else:
                params['demo_strong_ref'] = self.demo_strong_ref
        if self.demo_wink_ref:
            if hasattr(self.demo_wink_ref, 'to_alipay_dict'):
                params['demo_wink_ref'] = json.dumps(obj=self.demo_wink_ref.to_alipay_dict(), ensure_ascii=False, sort_keys=True, separators=(',', ':'))
            else:
