...
class AntfortuneFinresearchKnowledgeUploadRequest(object):
    ...
    def get_params(self):
        params = dict()
        params[P_METHOD] = 'antfortune.finresearch.knowledge.upload'
        params[P_VERSION] = self.version
        if self.biz_model:
            params[P_BIZ_CONTENT] = json.dumps(obj=self.biz_model.to_alipay_dict(), ensure_ascii=False, sort_keys=True, separators=(',', ':'))
        if self.file_info:
            if hasattr(self.file_info, 'to_alipay_dict'):
                params['file_info'] = json.dumps(obj=self.file_info.to_alipay_dict(), ensure_ascii=False, sort_keys=True, separators=(',', ':'))
            else:
