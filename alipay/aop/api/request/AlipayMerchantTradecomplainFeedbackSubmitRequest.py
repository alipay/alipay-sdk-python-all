...
class AlipayMerchantTradecomplainFeedbackSubmitRequest(object):
    ...
    def get_params(self):
        params = dict()
        params[P_METHOD] = 'alipay.merchant.tradecomplain.feedback.submit'
        params[P_VERSION] = self.version
        if self.biz_model:
            params[P_BIZ_CONTENT] = json.dumps(obj=self.biz_model.to_alipay_dict(), ensure_ascii=False, sort_keys=True, separators=(',', ':'))
        if self.complain_event_id:
            if hasattr(self.complain_event_id, 'to_alipay_dict'):
                params['complain_event_id'] = json.dumps(obj=self.complain_event_id.to_alipay_dict(), ensure_ascii=False, sort_keys=True, separators=(',', ':'))
            else:
                params['complain_event_id'] = self.complain_event_id
        if self.feedback_code:
            if hasattr(self.feedback_code, 'to_alipay_dict'):
                params['feedback_code'] = json.dumps(obj=self.feedback_code.to_alipay_dict(), ensure_ascii=False, sort_keys=True, separators=(',', ':'))
            else:
                params['feedback_code'] = self.feedback_code
        if self.feedback_content:
            if hasattr(self.feedback_content, 'to_alipay_dict'):
                params['feedback_content'] = json.dumps(obj=self.feedback_content.to_alipay_dict(), ensure_ascii=False, sort_keys=True, separators=(',', ':'))
            else:
                params['feedback_content'] = self.feedback_content
        if self.feedback_images:
            if hasattr(self.feedback_images, 'to_alipay_dict'):
                params['feedback_images'] = json.dumps(obj=self.feedback_images.to_alipay_dict(), ensure_ascii=False, sort_keys=True, separators=(',', ':'))
            else:
