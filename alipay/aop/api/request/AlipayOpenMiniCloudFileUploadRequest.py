...
class AlipayOpenMiniCloudFileUploadRequest(object):
    ...
    def get_params(self):
        params = dict()
        params[P_METHOD] = 'alipay.open.mini.cloud.file.upload'
        params[P_VERSION] = self.version
        if self.biz_model:
            params[P_BIZ_CONTENT] = json.dumps(obj=self.biz_model.to_alipay_dict(), ensure_ascii=False, sort_keys=True, separators=(',', ':'))
        if self.cloud_id:
            if hasattr(self.cloud_id, 'to_alipay_dict'):
                params['cloud_id'] = json.dumps(obj=self.cloud_id.to_alipay_dict(), ensure_ascii=False, sort_keys=True, separators=(',', ':'))
            else:
        ...
        if self.path:
            if hasattr(self.path, 'to_alipay_dict'):
                params['path'] = json.dumps(obj=self.path.to_alipay_dict(), ensure_ascii=False, sort_keys=True, separators=(',', ':'))
            else:
