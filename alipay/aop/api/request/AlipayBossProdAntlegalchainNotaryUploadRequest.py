...
class AlipayBossProdAntlegalchainNotaryUploadRequest(object):
    ...
    def get_params(self):
        params = dict()
        params[P_METHOD] = 'alipay.boss.prod.antlegalchain.notary.upload'
        params[P_VERSION] = self.version
        if self.biz_model:
            params[P_BIZ_CONTENT] = json.dumps(obj=self.biz_model.to_alipay_dict(), ensure_ascii=False, sort_keys=True, separators=(',', ':'))
        if self.app_code:
            if hasattr(self.app_code, 'to_alipay_dict'):
                params['app_code'] = json.dumps(obj=self.app_code.to_alipay_dict(), ensure_ascii=False, sort_keys=True, separators=(',', ':'))
            else:
                params['app_code'] = self.app_code
        if self.business_unique_id:
            if hasattr(self.business_unique_id, 'to_alipay_dict'):
                params['business_unique_id'] = json.dumps(obj=self.business_unique_id.to_alipay_dict(), ensure_ascii=False, sort_keys=True, separators=(',', ':'))
            else:
        ...
        if self.tenant:
            if hasattr(self.tenant, 'to_alipay_dict'):
                params['tenant'] = json.dumps(obj=self.tenant.to_alipay_dict(), ensure_ascii=False, sort_keys=True, separators=(',', ':'))
            else:
