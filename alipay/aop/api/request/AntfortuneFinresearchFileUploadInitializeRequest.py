...
class AntfortuneFinresearchFileUploadInitializeRequest(object):
    ...
    def get_params(self):
        params = dict()
        params[P_METHOD] = 'antfortune.finresearch.file.upload.initialize'
        params[P_VERSION] = self.version
        if self.biz_model:
            params[P_BIZ_CONTENT] = json.dumps(obj=self.biz_model.to_alipay_dict(), ensure_ascii=False, sort_keys=True, separators=(',', ':'))
        if self.bu_unique_id:
            if hasattr(self.bu_unique_id, 'to_alipay_dict'):
                params['bu_unique_id'] = json.dumps(obj=self.bu_unique_id.to_alipay_dict(), ensure_ascii=False, sort_keys=True, separators=(',', ':'))
            else:
