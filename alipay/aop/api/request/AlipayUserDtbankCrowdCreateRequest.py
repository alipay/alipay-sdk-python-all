...
class AlipayUserDtbankCrowdCreateRequest(object):
    ...
    def get_params(self):
        params = dict()
        params[P_METHOD] = 'alipay.user.dtbank.crowd.create'
        params[P_VERSION] = self.version
        if self.biz_model:
            params[P_BIZ_CONTENT] = json.dumps(obj=self.biz_model.to_alipay_dict(), ensure_ascii=False, sort_keys=True, separators=(',', ':'))
        if self.crowd_name:
            if hasattr(self.crowd_name, 'to_alipay_dict'):
                params['crowd_name'] = json.dumps(obj=self.crowd_name.to_alipay_dict(), ensure_ascii=False, sort_keys=True, separators=(',', ':'))
            else:
