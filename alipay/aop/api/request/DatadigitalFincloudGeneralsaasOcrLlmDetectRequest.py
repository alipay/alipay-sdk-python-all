...
class DatadigitalFincloudGeneralsaasOcrLlmDetectRequest(object):
    ...
    def get_params(self):
        params = dict()
        params[P_METHOD] = 'datadigital.fincloud.generalsaas.ocr.llm.detect'
        ...
        if self.prompt:
            if hasattr(self.prompt, 'to_alipay_dict'):
                params['prompt'] = json.dumps(obj=self.prompt.to_alipay_dict(), ensure_ascii=False, sort_keys=True, separators=(',', ':'))
            else:
