...
class AlipayMerchantTradeFileUploadRequest(object):
    ...
    def get_params(self):
        params = dict()
        params[P_METHOD] = 'alipay.merchant.trade.file.upload'
        params[P_VERSION] = self.version
        if self.biz_model:
            params[P_BIZ_CONTENT] = json.dumps(obj=self.biz_model.to_alipay_dict(), ensure_ascii=False, sort_keys=True, separators=(',', ':'))
        if self.data_dim:
            if hasattr(self.data_dim, 'to_alipay_dict'):
                params['data_dim'] = json.dumps(obj=self.data_dim.to_alipay_dict(), ensure_ascii=False, sort_keys=True, separators=(',', ':'))
            else:
                params['data_dim'] = self.data_dim
        if self.file_name:
        ...
        if self.isv_app_id:
            if hasattr(self.isv_app_id, 'to_alipay_dict'):
                params['isv_app_id'] = json.dumps(obj=self.isv_app_id.to_alipay_dict(), ensure_ascii=False, sort_keys=True, separators=(',', ':'))
            else:
