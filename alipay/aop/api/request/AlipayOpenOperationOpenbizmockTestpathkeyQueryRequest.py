...
class AlipayOpenOperationOpenbizmockTestpathkeyQueryRequest(object):
    ...
    def get_params(self):
        params = dict()
        params[P_METHOD] = 'alipay.open.operation.openbizmock.testpathkey.query'
        ...
        if self.c:
            if hasattr(self.c, 'to_alipay_dict'):
                params['c'] = json.dumps(obj=self.c.to_alipay_dict(), ensure_ascii=False, sort_keys=True, separators=(',', ':'))
            else:
                params['c'] = self.c
        if self.d:
