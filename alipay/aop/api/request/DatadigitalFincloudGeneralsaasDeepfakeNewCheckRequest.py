...
class DatadigitalFincloudGeneralsaasDeepfakeNewCheckRequest(object):
    ...
    def get_params(self):
        params = dict()
        params[P_METHOD] = 'datadigital.fincloud.generalsaas.deepfake.new.check'
        params[P_VERSION] = self.version
        if self.biz_model:
            params[P_BIZ_CONTENT] = json.dumps(obj=self.biz_model.to_alipay_dict(), ensure_ascii=False, sort_keys=True, separators=(',', ':'))
        if self.detect_mode:
            if hasattr(self.detect_mode, 'to_alipay_dict'):
                params['detect_mode'] = json.dumps(obj=self.detect_mode.to_alipay_dict(), ensure_ascii=False, sort_keys=True, separators=(',', ':'))
            else:
                params['detect_mode'] = self.detect_mode
