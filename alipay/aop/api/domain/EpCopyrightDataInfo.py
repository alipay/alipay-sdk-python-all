...
from alipay.aop.api.domain.EpCopyrightInfo import EpCopyrightInfo


class EpCopyrightDataInfo(object):
    ...
    def hits(self, value):
        if isinstance(value, list):
            self._hits = list()
            for i in value:
                if isinstance(i, EpCopyrightInfo):
                    self._hits.append(i)
                else:
                    self._hits.append(EpCopyrightInfo.from_alipay_dict(i))
    @property
    def total(self):
        return self._total
