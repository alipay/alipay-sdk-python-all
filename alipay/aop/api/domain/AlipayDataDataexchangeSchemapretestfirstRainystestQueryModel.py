...
class AlipayDataDataexchangeSchemapretestfirstRainystestQueryModel(object):
    ...
    def demo_date(self, value):
        if isinstance(value, list):
            self._demo_date = list()
            for i in value:
                if isinstance(i, RainyAAA):
                    self._demo_date.append(i)
                else:
                    self._demo_date.append(RainyAAA.from_alipay_dict(i))
    @property
    ...
    def to_alipay_dict(self):
        ...
        if self.demo_date:
            if isinstance(self.demo_date, list):
                for i in range(0, len(self.demo_date)):
                    element = self.demo_date[i]
                    if hasattr(element, 'to_alipay_dict'):
                        self.demo_date[i] = element.to_alipay_dict()
    ...
    def from_alipay_dict(d):
        if not d:
            return None
        o = AlipayDataDataexchangeSchemapretestfirstRainystestQueryModel()
