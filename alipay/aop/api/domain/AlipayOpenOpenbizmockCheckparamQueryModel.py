...
from alipay.aop.api.domain.ComplexTestAll import ComplexTestAll
from alipay.aop.api.domain.ComplexTestAll import ComplexTestAll


class AlipayOpenOpenbizmockCheckparamQueryModel(object):
    ...
    def complex_list_must(self, value):
        if isinstance(value, list):
            ...
            for i in value:
                if isinstance(i, ComplexTestAll):
                    self._complex_list_must.append(i)
                else:
                    self._complex_list_must.append(ComplexTestAll.from_alipay_dict(i))
    @property
    ...
    def complex_must(self, value):
        if isinstance(value, ComplexTestAll):
            self._complex_must = value
        else:
            self._complex_must = ComplexTestAll.from_alipay_dict(value)
    @property
    ...
    def from_alipay_dict(d):
        if not d:
            return None
        o = AlipayOpenOpenbizmockCheckparamQueryModel()
