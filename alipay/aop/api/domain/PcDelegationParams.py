...
class PcDelegationParams(object):
    ...
    def external_tradeno_list(self, value):
        if isinstance(value, list):
            self._external_tradeno_list = list()
            for i in value:
                self._external_tradeno_list.append(i)
    @property
    ...
    def to_alipay_dict(self):
        ...
        if self.external_tradeno_list:
            if isinstance(self.external_tradeno_list, list):
                for i in range(0, len(self.external_tradeno_list)):
                    element = self.external_tradeno_list[i]
                    if hasattr(element, 'to_alipay_dict'):
                        self.external_tradeno_list[i] = element.to_alipay_dict()
