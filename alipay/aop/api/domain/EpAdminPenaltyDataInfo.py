...
from alipay.aop.api.domain.EpAdminPenaltyInfo import EpAdminPenaltyInfo


class EpAdminPenaltyDataInfo(object):
    ...
    def hits(self, value):
        if isinstance(value, list):
            self._hits = list()
            for i in value:
                if isinstance(i, EpAdminPenaltyInfo):
                    self._hits.append(i)
                else:
                    self._hits.append(EpAdminPenaltyInfo.from_alipay_dict(i))
    @property
    def total(self):
        return self._total
