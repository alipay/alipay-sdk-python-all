...
from alipay.aop.api.domain.EpReginfoChangeInfo import EpReginfoChangeInfo


class EpReginfoChangeDataInfo(object):
    ...
    def hits(self, value):
        if isinstance(value, list):
            self._hits = list()
            for i in value:
                if isinstance(i, EpReginfoChangeInfo):
                    self._hits.append(i)
                else:
                    self._hits.append(EpReginfoChangeInfo.from_alipay_dict(i))
    @property
    def total(self):
        return self._total
