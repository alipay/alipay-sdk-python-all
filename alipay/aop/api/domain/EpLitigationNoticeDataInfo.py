...
from alipay.aop.api.domain.EpLitigationNoticeInfo import EpLitigationNoticeInfo


class EpLitigationNoticeDataInfo(object):
    ...
    def hits(self, value):
        if isinstance(value, list):
            self._hits = list()
            for i in value:
                if isinstance(i, EpLitigationNoticeInfo):
                    self._hits.append(i)
                else:
                    self._hits.append(EpLitigationNoticeInfo.from_alipay_dict(i))
    @property
    def total(self):
        return self._total
