...
class AlipayCommerceAcommunicationCreditphonePreconsultSubmitModel(object):
    ...
    @property
    def inst_pid_list(self):
        return self._inst_pid_list

    @inst_pid_list.setter
    def inst_pid_list(self, value):
        if isinstance(value, list):
            self._inst_pid_list = list()
            for i in value:
                self._inst_pid_list.append(i)
    @property
    ...
    def to_alipay_dict(self):
        ...
        if self.inst_pid_list:
            if isinstance(self.inst_pid_list, list):
                for i in range(0, len(self.inst_pid_list)):
                    element = self.inst_pid_list[i]
                    if hasattr(element, 'to_alipay_dict'):
                        self.inst_pid_list[i] = element.to_alipay_dict()
            if hasattr(self.inst_pid_list, 'to_alipay_dict'):
                params['inst_pid_list'] = self.inst_pid_list.to_alipay_dict()
            else:
                params['inst_pid_list'] = self.inst_pid_list
    ...
    def from_alipay_dict(d):
        if not d:
            return None
        o = AlipayCommerceAcommunicationCreditphonePreconsultSubmitModel()
        ...
        if 'inst_pid_list' in d:
            o.inst_pid_list = d['inst_pid_list']
