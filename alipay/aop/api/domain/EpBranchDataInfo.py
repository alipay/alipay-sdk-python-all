...
from alipay.aop.api.domain.EpBranchDetailInfo import EpBranchDetailInfo


class EpBranchDataInfo(object):
    ...
    def hits(self, value):
        if isinstance(value, list):
            self._hits = list()
            for i in value:
                if isinstance(i, EpBranchDetailInfo):
                    self._hits.append(i)
                else:
                    self._hits.append(EpBranchDetailInfo.from_alipay_dict(i))
    @property
    def total(self):
        return self._total
