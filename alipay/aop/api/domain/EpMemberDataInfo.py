...
from alipay.aop.api.domain.EpMemberInfo import EpMemberInfo


class EpMemberDataInfo(object):
    ...
    def hits(self, value):
        if isinstance(value, list):
            self._hits = list()
            for i in value:
                if isinstance(i, EpMemberInfo):
                    self._hits.append(i)
                else:
                    self._hits.append(EpMemberInfo.from_alipay_dict(i))
    @property
    def total(self):
        return self._total
