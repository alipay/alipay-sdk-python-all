...
from alipay.aop.api.domain.MultiDimAuthAmountQuery import MultiDimAuthAmountQuery
from alipay.aop.api.domain.QueryInfo import QueryInfo


class AlipayBossBaseAntauthorizeMultiQueryModel(object):
    ...
    def query(self, value):
        if isinstance(value, MultiDimAuthAmountQuery):
            self._query = value
        else:
            self._query = MultiDimAuthAmountQuery.from_alipay_dict(value)
    @property
    def query_info(self):
    ...
    def from_alipay_dict(d):
        if not d:
            return None
        o = AlipayBossBaseAntauthorizeMultiQueryModel()
        if 'query' in d:
            o.query = d['query']
        if 'query_info' in d:
