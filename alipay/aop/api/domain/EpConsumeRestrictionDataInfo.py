...
from alipay.aop.api.domain.EpConsumeRestrictionInfo import EpConsumeRestrictionInfo


class EpConsumeRestrictionDataInfo(object):
    ...
    def hits(self, value):
        if isinstance(value, list):
            self._hits = list()
            for i in value:
                if isinstance(i, EpConsumeRestrictionInfo):
                    self._hits.append(i)
                else:
                    self._hits.append(EpConsumeRestrictionInfo.from_alipay_dict(i))
    @property
    def total(self):
        return self._total
    ...
    def from_alipay_dict(d):
        if not d:
            return None
        o = EpConsumeRestrictionDataInfo()
        if 'hits' in d:
            o.hits = d['hits']
        if 'total' in d:
            o.total = d['total']
