...
from alipay.aop.api.domain.EpTrademarkInfo import EpTrademarkInfo


class EpTrademarkDataInfo(object):
    ...
    def hits(self, value):
        if isinstance(value, list):
            self._hits = list()
            for i in value:
                if isinstance(i, EpTrademarkInfo):
                    self._hits.append(i)
                else:
                    self._hits.append(EpTrademarkInfo.from_alipay_dict(i))
    @property
    def total(self):
        return self._total
