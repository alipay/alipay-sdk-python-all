...
from alipay.aop.api.domain.LubBusinessCopyLicenseInfo import LubBusinessCopyLicenseInfo
from alipay.aop.api.domain.LubCopyContactWayInfo import LubCopyContactWayInfo
from alipay.aop.api.domain.LubLegalCopyPersonInfo import LubLegalCopyPersonInfo
from alipay.aop.api.domain.LubStoreCopyAddressInfo import LubStoreCopyAddressInfo


class LubUnifiedShopCopyListQueryItem(object):
    ...
    def business_license_info(self, value):
        if isinstance(value, LubBusinessCopyLicenseInfo):
            self._business_license_info = value
        else:
            self._business_license_info = LubBusinessCopyLicenseInfo.from_alipay_dict(value)
    @property
    ...
    def contact_ways(self, value):
        if isinstance(value, list):
            ...
            for i in value:
                if isinstance(i, LubCopyContactWayInfo):
                    self._contact_ways.append(i)
                else:
                    self._contact_ways.append(LubCopyContactWayInfo.from_alipay_dict(i))
    @property
    def copy_id(self):
    ...
    def legal_person_info(self, value):
        if isinstance(value, LubLegalCopyPersonInfo):
            self._legal_person_info = value
        else:
            self._legal_person_info = LubLegalCopyPersonInfo.from_alipay_dict(value)
    @property
    def pid(self):
        return self._pid
    ...
    def store_address_info(self, value):
        if isinstance(value, LubStoreCopyAddressInfo):
            self._store_address_info = value
        else:
            self._store_address_info = LubStoreCopyAddressInfo.from_alipay_dict(value)
    @property
    def store_id(self):
    ...
    def from_alipay_dict(d):
        if not d:
            return None
        o = LubUnifiedShopCopyListQueryItem()
        if 'brand_id' in d:
