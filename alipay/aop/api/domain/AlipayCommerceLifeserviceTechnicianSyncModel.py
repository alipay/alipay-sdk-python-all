...
class AlipayCommerceLifeserviceTechnicianSyncModel(object):
    ...
    def status(self, value):
        self._status = value
    @property
    def technician_attrs(self):
        return self._technician_attrs

    @technician_attrs.setter
    def technician_attrs(self, value):
        if isinstance(value, list):
            self._technician_attrs = list()
            for i in value:
                if isinstance(i, LifeServiceAttr):
                    self._technician_attrs.append(i)
                else:
                    self._technician_attrs.append(LifeServiceAttr.from_alipay_dict(i))
    @property
    ...
    def to_alipay_dict(self):
        ...
        if self.technician_attrs:
            if isinstance(self.technician_attrs, list):
                for i in range(0, len(self.technician_attrs)):
                    element = self.technician_attrs[i]
                    if hasattr(element, 'to_alipay_dict'):
                        self.technician_attrs[i] = element.to_alipay_dict()
            if hasattr(self.technician_attrs, 'to_alipay_dict'):
                params['technician_attrs'] = self.technician_attrs.to_alipay_dict()
            else:
                params['technician_attrs'] = self.technician_attrs
    ...
    def from_alipay_dict(d):
        if not d:
            return None
        o = AlipayCommerceLifeserviceTechnicianSyncModel()
        ...
        if 'status' in d:
            o.status = d['status']
        if 'technician_attrs' in d:
            o.technician_attrs = d['technician_attrs']
