...
from alipay.aop.api.domain.BizEntity import BizEntity


class AlipayCommerceRentAgentChatModel(object):
    ...
    def biz_entity_ids(self, value):
        if isinstance(value, list):
            ...
            for i in value:
                if isinstance(i, BizEntity):
                    self._biz_entity_ids.append(i)
                else:
                    self._biz_entity_ids.append(BizEntity.from_alipay_dict(i))
    @property
    def biz_type(self):
    ...
    def from_alipay_dict(d):
        if not d:
            return None
        o = AlipayCommerceRentAgentChatModel()
