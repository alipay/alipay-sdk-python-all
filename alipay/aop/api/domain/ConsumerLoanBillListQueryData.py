...
from alipay.aop.api.domain.ConsumerLoanBillInfo import ConsumerLoanBillInfo
from alipay.aop.api.domain.ConsumerLoanNotMatchData import ConsumerLoanNotMatchData


class ConsumerLoanBillListQueryData(object):
    ...
    def bill_data(self, value):
        if isinstance(value, ConsumerLoanBillInfo):
            self._bill_data = value
        else:
            self._bill_data = ConsumerLoanBillInfo.from_alipay_dict(value)
    @property
    ...
    def from_alipay_dict(d):
        if not d:
            return None
        o = ConsumerLoanBillListQueryData()
        if 'bill_data' in d:
