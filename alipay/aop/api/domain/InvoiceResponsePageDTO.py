...
class InvoiceResponsePageDTO(object):
    ...
    def datas(self, value):
        if isinstance(value, list):
            self._datas = list()
            for i in value:
                if isinstance(i, InvoiceAllResponseDTO):
                    self._datas.append(i)
                else:
                    self._datas.append(InvoiceAllResponseDTO.from_alipay_dict(i))
    @property
    def page_size(self):
