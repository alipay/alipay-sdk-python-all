...
from alipay.aop.api.domain.EpEquityPledgeInfo import EpEquityPledgeInfo


class EpEquityPledgeDataInfo(object):
    ...
    def hits(self, value):
        if isinstance(value, list):
            self._hits = list()
            for i in value:
                if isinstance(i, EpEquityPledgeInfo):
                    self._hits.append(i)
                else:
                    self._hits.append(EpEquityPledgeInfo.from_alipay_dict(i))
    @property
    def total(self):
        return self._total
