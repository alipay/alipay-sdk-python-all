...
from alipay.aop.api.domain.InvoiceBillDTO import InvoiceBillDTO


class ResponseModelForInvoiceBillDTO(object):
    ...
    def data(self, value):
        if isinstance(value, list):
            self._data = list()
            for i in value:
                if isinstance(i, InvoiceBillDTO):
                    self._data.append(i)
                else:
                    self._data.append(InvoiceBillDTO.from_alipay_dict(i))
    @property
    def error_code(self):
    ...
    def from_alipay_dict(d):
        if not d:
            return None
        o = ResponseModelForInvoiceBillDTO()
        if 'data' in d:
            o.data = d['data']
        if 'error_code' in d:
