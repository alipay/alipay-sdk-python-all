...
from alipay.aop.api.domain.ZmepBiddingInfo import ZmepBiddingInfo


class ZmepBiddingDataInfo(object):
    ...
    def hits(self, value):
        if isinstance(value, list):
            self._hits = list()
            for i in value:
                if isinstance(i, ZmepBiddingInfo):
                    self._hits.append(i)
                else:
                    self._hits.append(ZmepBiddingInfo.from_alipay_dict(i))
    @property
    def total(self):
        return self._total
