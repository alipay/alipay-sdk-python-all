...
from alipay.aop.api.domain.ZmEpBondRatingInfo import ZmEpBondRatingInfo


class ZmEpBondRatingDataInfo(object):
    ...
    def hits(self, value):
        if isinstance(value, list):
            self._hits = list()
            for i in value:
                if isinstance(i, ZmEpBondRatingInfo):
                    self._hits.append(i)
                else:
                    self._hits.append(ZmEpBondRatingInfo.from_alipay_dict(i))
    @property
    def total(self):
        return self._total
