...
from alipay.aop.api.domain.VoucherSendGuideModify import VoucherSendGuideModify
from alipay.aop.api.domain.VoucherUseGuideModify import VoucherUseGuideModify
...
class CustomerGuideModify(object):
    ...
    def voucher_send_guide(self, value):
        if isinstance(value, VoucherSendGuideModify):
            self._voucher_send_guide = value
        else:
            self._voucher_send_guide = VoucherSendGuideModify.from_alipay_dict(value)
    @property
    ...
    def voucher_use_guide(self, value):
        if isinstance(value, VoucherUseGuideModify):
            self._voucher_use_guide = value
        else:
            self._voucher_use_guide = VoucherUseGuideModify.from_alipay_dict(value)
