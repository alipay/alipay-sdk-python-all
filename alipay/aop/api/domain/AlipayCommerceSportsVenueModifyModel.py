...
from alipay.aop.api.domain.SubVenueModifyInfo import SubVenueModifyInfo


class AlipayCommerceSportsVenueModifyModel(object):
    ...
    def sub_venue_list(self, value):
        if isinstance(value, list):
            ...
            for i in value:
                if isinstance(i, SubVenueModifyInfo):
                    self._sub_venue_list.append(i)
                else:
                    self._sub_venue_list.append(SubVenueModifyInfo.from_alipay_dict(i))
    @property
    def tag_list(self):
    ...
    def from_alipay_dict(d):
        if not d:
            return None
        o = AlipayCommerceSportsVenueModifyModel()
        if 'address' in d:
            o.address = d['address']
        if 'area_code' in d:
