...
from alipay.aop.api.domain.CategoryRiskDTO import CategoryRiskDTO
from alipay.aop.api.domain.QuotaGradientRuleDTO import QuotaGradientRuleDTO


class RiskConfigDTO(object):
    ...
    def category_risks(self, value):
        if isinstance(value, list):
            ...
            for i in value:
                if isinstance(i, CategoryRiskDTO):
                    self._category_risks.append(i)
                else:
                    self._category_risks.append(CategoryRiskDTO.from_alipay_dict(i))
    @property
    ...
    def quota_gradient_rule(self, value):
        if isinstance(value, QuotaGradientRuleDTO):
            self._quota_gradient_rule = value
        else:
            self._quota_gradient_rule = QuotaGradientRuleDTO.from_alipay_dict(value)
