...
from alipay.aop.api.domain.EpCourtNoticeInfo import EpCourtNoticeInfo


class EpCourtNoticeDataInfo(object):
    ...
    def hits(self, value):
        if isinstance(value, list):
            self._hits = list()
            for i in value:
                if isinstance(i, EpCourtNoticeInfo):
                    self._hits.append(i)
                else:
                    self._hits.append(EpCourtNoticeInfo.from_alipay_dict(i))
    @property
    def total(self):
        return self._total
