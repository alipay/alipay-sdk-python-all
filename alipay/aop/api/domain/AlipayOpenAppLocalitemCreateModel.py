...
from alipay.aop.api.domain.LocalItemSkuCreateVO import LocalItemSkuCreateVO
from alipay.aop.api.domain.TimeRangeStructVO import TimeRangeStructVO


class AlipayOpenAppLocalitemCreateModel(object):
    ...
    def skus(self, value):
        if isinstance(value, list):
            self._skus = list()
            for i in value:
                if isinstance(i, LocalItemSkuCreateVO):
                    self._skus.append(i)
                else:
                    self._skus.append(LocalItemSkuCreateVO.from_alipay_dict(i))
    @property
    def sold_time(self):
