...
from alipay.aop.api.domain.ZmepAnnualReportInfo import ZmepAnnualReportInfo


class ZmepAnnualReportDataInfo(object):
    ...
    def hits(self, value):
        if isinstance(value, list):
            self._hits = list()
            for i in value:
                if isinstance(i, ZmepAnnualReportInfo):
                    self._hits.append(i)
                else:
                    self._hits.append(ZmepAnnualReportInfo.from_alipay_dict(i))
    @property
    def total(self):
        return self._total
