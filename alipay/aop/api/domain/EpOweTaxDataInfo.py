...
from alipay.aop.api.domain.EpOweTaxInfo import EpOweTaxInfo


class EpOweTaxDataInfo(object):
    ...
    def hits(self, value):
        if isinstance(value, list):
            self._hits = list()
            for i in value:
                if isinstance(i, EpOweTaxInfo):
                    self._hits.append(i)
                else:
                    self._hits.append(EpOweTaxInfo.from_alipay_dict(i))
    @property
    def total(self):
        return self._total
