...
from alipay.aop.api.domain.EpPatentInfo import EpPatentInfo


class EpPatentDataInfo(object):
    ...
    def hits(self, value):
        if isinstance(value, list):
            self._hits = list()
            for i in value:
                if isinstance(i, EpPatentInfo):
                    self._hits.append(i)
                else:
                    self._hits.append(EpPatentInfo.from_alipay_dict(i))
    @property
    def total(self):
        return self._total
