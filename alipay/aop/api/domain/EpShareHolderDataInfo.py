...
from alipay.aop.api.domain.EpShareHolderInfo import EpShareHolderInfo


class EpShareHolderDataInfo(object):
    ...
    def hits(self, value):
        if isinstance(value, list):
            self._hits = list()
            for i in value:
                if isinstance(i, EpShareHolderInfo):
                    self._hits.append(i)
                else:
                    self._hits.append(EpShareHolderInfo.from_alipay_dict(i))
    @property
    def total(self):
        return self._total
