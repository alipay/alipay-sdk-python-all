...
from alipay.aop.api.domain.ZmepCertificateInfo import ZmepCertificateInfo


class ZmepCertificateDataInfo(object):
    ...
    def hits(self, value):
        if isinstance(value, list):
            self._hits = list()
            for i in value:
                if isinstance(i, ZmepCertificateInfo):
                    self._hits.append(i)
                else:
                    self._hits.append(ZmepCertificateInfo.from_alipay_dict(i))
    @property
    def total(self):
        return self._total
