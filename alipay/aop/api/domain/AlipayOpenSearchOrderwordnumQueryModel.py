...
class AlipayOpenSearchOrderwordnumQueryModel(object):
    ...
    def biz_data(self, value):
        if isinstance(value, SearchOrderBaseKeyWordNumRequest):
            self._biz_data = value
        else:
            self._biz_data = SearchOrderBaseKeyWordNumRequest.from_alipay_dict(value)
    @property
    def biz_type(self):
    ...
    def from_alipay_dict(d):
        if not d:
            return None
        o = AlipayOpenSearchOrderwordnumQueryModel()
        if 'biz_data' in d:
