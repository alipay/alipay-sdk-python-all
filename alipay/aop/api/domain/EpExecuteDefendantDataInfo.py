...
from alipay.aop.api.domain.EpExecuteDefendantInfo import EpExecuteDefendantInfo


class EpExecuteDefendantDataInfo(object):
    ...
    def hits(self, value):
        if isinstance(value, list):
            self._hits = list()
            for i in value:
                if isinstance(i, EpExecuteDefendantInfo):
                    self._hits.append(i)
                else:
                    self._hits.append(EpExecuteDefendantInfo.from_alipay_dict(i))
    @property
    def total(self):
        return self._total
