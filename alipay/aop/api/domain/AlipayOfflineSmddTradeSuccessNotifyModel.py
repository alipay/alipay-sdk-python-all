...
from alipay.aop.api.domain.PayInfoBean import PayInfoBean


class AlipayOfflineSmddTradeSuccessNotifyModel(object):
    ...
    def pay_info(self, value):
        if isinstance(value, PayInfoBean):
            self._pay_info = value
        else:
            self._pay_info = PayInfoBean.from_alipay_dict(value)
    @property
    def shop_id(self):
    ...
    def from_alipay_dict(d):
        if not d:
            return None
        o = AlipayOfflineSmddTradeSuccessNotifyModel()
        if 'buyer_id' in d:
