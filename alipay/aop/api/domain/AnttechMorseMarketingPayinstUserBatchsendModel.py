...
from alipay.aop.api.domain.PayInstUser import PayInstUser


class AnttechMorseMarketingPayinstUserBatchsendModel(object):
    ...
    def user_list(self, value):
        if isinstance(value, list):
            ...
            for i in value:
                if isinstance(i, PayInstUser):
                    self._user_list.append(i)
                else:
                    self._user_list.append(PayInstUser.from_alipay_dict(i))
    ...
    def from_alipay_dict(d):
        if not d:
            return None
        o = AnttechMorseMarketingPayinstUserBatchsendModel()
        if 'user_list' in d:
