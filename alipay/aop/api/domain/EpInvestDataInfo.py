...
from alipay.aop.api.domain.EpInvestInfo import EpInvestInfo


class EpInvestDataInfo(object):
    ...
    def hits(self, value):
        if isinstance(value, list):
            self._hits = list()
            for i in value:
                if isinstance(i, EpInvestInfo):
                    self._hits.append(i)
                else:
                    self._hits.append(EpInvestInfo.from_alipay_dict(i))
    @property
    def total(self):
        return self._total
