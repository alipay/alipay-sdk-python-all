...
from alipay.aop.api.domain.EpAdminLicenseInfo import EpAdminLicenseInfo


class EpAdminLicenseDataInfo(object):
    ...
    def hits(self, value):
        if isinstance(value, list):
            self._hits = list()
            for i in value:
                if isinstance(i, EpAdminLicenseInfo):
                    self._hits.append(i)
                else:
                    self._hits.append(EpAdminLicenseInfo.from_alipay_dict(i))
    @property
    def total(self):
        return self._total
