...
from alipay.aop.api.domain.ShopTimePeriod import ShopTimePeriod


class AShopBusinessTime(object):
    ...
    def times(self, value):
        if isinstance(value, list):
            self._times = list()
            for i in value:
                if isinstance(i, ShopTimePeriod):
                    self._times.append(i)
                else:
                    self._times.append(ShopTimePeriod.from_alipay_dict(i))
