...
from alipay.aop.api.domain.EpFinalCaseInfo import EpFinalCaseInfo


class EpFinalCaseDataInfo(object):
    ...
    def hits(self, value):
        if isinstance(value, list):
            self._hits = list()
            for i in value:
                if isinstance(i, EpFinalCaseInfo):
                    self._hits.append(i)
                else:
                    self._hits.append(EpFinalCaseInfo.from_alipay_dict(i))
    @property
    def total(self):
        return self._total
