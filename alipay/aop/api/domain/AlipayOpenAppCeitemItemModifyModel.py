...
from alipay.aop.api.domain.CeAppItemAttrVO import CeAppItemAttrVO
from alipay.aop.api.domain.CeItemDescInfoModifyVO import CeItemDescInfoModifyVO
from alipay.aop.api.domain.CeItemSkuVO import CeItemSkuVO


class AlipayOpenAppCeitemItemModifyModel(object):
    ...
    def attrs(self, value):
        if isinstance(value, list):
            self._attrs = list()
            for i in value:
                if isinstance(i, CeAppItemAttrVO):
                    self._attrs.append(i)
                else:
                    self._attrs.append(CeAppItemAttrVO.from_alipay_dict(i))
    @property
    def barcode(self):
    ...
    def desc_info(self, value):
        if isinstance(value, CeItemDescInfoModifyVO):
            self._desc_info = value
        else:
            self._desc_info = CeItemDescInfoModifyVO.from_alipay_dict(value)
    @property
    def head_img(self):
    ...
    def skus(self, value):
        if isinstance(value, list):
            self._skus = list()
            for i in value:
                if isinstance(i, CeItemSkuVO):
                    self._skus.append(i)
                else:
                    self._skus.append(CeItemSkuVO.from_alipay_dict(i))
    @property
    def stock_num(self):
    ...
    def from_alipay_dict(d):
        if not d:
            return None
        o = AlipayOpenAppCeitemItemModifyModel()
        if 'attrs' in d:
            o.attrs = d['attrs']
        if 'barcode' in d:
            o.barcode = d['barcode']
