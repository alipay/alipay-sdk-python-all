...
from alipay.aop.api.domain.EpICPRegistrationInfo import EpICPRegistrationInfo


class EpICPRegistrationDataInfo(object):
    ...
    def hits(self, value):
        if isinstance(value, list):
            self._hits = list()
            for i in value:
                if isinstance(i, EpICPRegistrationInfo):
                    self._hits.append(i)
                else:
                    self._hits.append(EpICPRegistrationInfo.from_alipay_dict(i))
    @property
    def total(self):
        return self._total
