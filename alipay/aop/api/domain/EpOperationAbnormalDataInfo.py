...
from alipay.aop.api.domain.EpOperationAbnormalInfo import EpOperationAbnormalInfo


class EpOperationAbnormalDataInfo(object):
    ...
    def hits(self, value):
        if isinstance(value, list):
            self._hits = list()
            for i in value:
                if isinstance(i, EpOperationAbnormalInfo):
                    self._hits.append(i)
                else:
                    self._hits.append(EpOperationAbnormalInfo.from_alipay_dict(i))
    @property
    def total(self):
        return self._total
