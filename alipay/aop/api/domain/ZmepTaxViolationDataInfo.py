...
from alipay.aop.api.domain.ZmepTaxViolationInfo import ZmepTaxViolationInfo


class ZmepTaxViolationDataInfo(object):
    ...
    def hits(self, value):
        if isinstance(value, list):
            self._hits = list()
            for i in value:
                if isinstance(i, ZmepTaxViolationInfo):
                    self._hits.append(i)
                else:
                    self._hits.append(ZmepTaxViolationInfo.from_alipay_dict(i))
    @property
    def total(self):
        return self._total
