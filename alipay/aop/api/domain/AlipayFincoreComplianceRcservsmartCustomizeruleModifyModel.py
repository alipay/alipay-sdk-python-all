...
from alipay.aop.api.domain.CustomizeRuleResult import CustomizeRuleResult


class AlipayFincoreComplianceRcservsmartCustomizeruleModifyModel(object):
    ...
    def rcsmart_biz_request(self, value):
        if isinstance(value, CustomizeRuleResult):
            self._rcsmart_biz_request = value
        else:
            self._rcsmart_biz_request = CustomizeRuleResult.from_alipay_dict(value)
    ...
    def from_alipay_dict(d):
        if not d:
            return None
        o = AlipayFincoreComplianceRcservsmartCustomizeruleModifyModel()
        if 'app_info' in d:
