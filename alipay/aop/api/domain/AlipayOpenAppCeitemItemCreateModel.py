...
from alipay.aop.api.domain.CeItemAttrVO import CeItemAttrVO
from alipay.aop.api.domain.CeItemDescInfoVO import CeItemDescInfoVO
from alipay.aop.api.domain.CeItemSkuCreateVO import CeItemSkuCreateVO


class AlipayOpenAppCeitemItemCreateModel(object):
    ...
    def attrs(self, value):
        if isinstance(value, list):
            self._attrs = list()
            for i in value:
                if isinstance(i, CeItemAttrVO):
                    self._attrs.append(i)
                else:
                    self._attrs.append(CeItemAttrVO.from_alipay_dict(i))
    @property
    ...
    def desc_info(self, value):
        if isinstance(value, CeItemDescInfoVO):
            self._desc_info = value
        else:
            self._desc_info = CeItemDescInfoVO.from_alipay_dict(value)
    @property
    def head_img(self):
    ...
    def skus(self, value):
        if isinstance(value, list):
            self._skus = list()
            for i in value:
                if isinstance(i, CeItemSkuCreateVO):
                    self._skus.append(i)
                else:
                    self._skus.append(CeItemSkuCreateVO.from_alipay_dict(i))
    @property
    def stock_num(self):
    ...
    def from_alipay_dict(d):
        if not d:
            return None
        o = AlipayOpenAppCeitemItemCreateModel()
        if 'attrs' in d:
            o.attrs = d['attrs']
