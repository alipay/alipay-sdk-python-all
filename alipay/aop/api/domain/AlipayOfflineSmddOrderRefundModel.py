...
from alipay.aop.api.domain.RefundInfoBean import RefundInfoBean


class AlipayOfflineSmddOrderRefundModel(object):
    ...
    def refund_info(self, value):
        if isinstance(value, RefundInfoBean):
            self._refund_info = value
        else:
            self._refund_info = RefundInfoBean.from_alipay_dict(value)
    @property
    def shop_id(self):
    ...
    def from_alipay_dict(d):
        if not d:
            return None
        o = AlipayOfflineSmddOrderRefundModel()
        if 'buyer_id' in d:
