#!/usr/bin/env python
# -*- coding: utf-8 -*-
import json

from alipay.aop.api.constant.ParamConstants import *
from alipay.aop.api.domain.BenefitUseVO import BenefitUseVO


class AlipayVoyagerMarketingRedeemModel(object):

    def __init__(self):
        self._benefit_use_infos = None
        self._biz_date = None
        self._biz_no = None
        self._freeze_order_id = None
        self._industry = None
        self._open_id = None
        self._user_id = None

    @property
    def benefit_use_infos(self):
        return self._benefit_use_infos

    @benefit_use_infos.setter
    def benefit_use_infos(self, value):
        if isinstance(value, list):
            self._benefit_use_infos = list()
            for i in value:
                if isinstance(i, BenefitUseVO):
                    self._benefit_use_infos.append(i)
                else:
                    self._benefit_use_infos.append(BenefitUseVO.from_alipay_dict(i))
    @property
    def biz_date(self):
        return self._biz_date

    @biz_date.setter
    def biz_date(self, value):
        self._biz_date = value
    @property
    def biz_no(self):
        return self._biz_no

    @biz_no.setter
    def biz_no(self, value):
        self._biz_no = value
    @property
    def freeze_order_id(self):
        return self._freeze_order_id

    @freeze_order_id.setter
    def freeze_order_id(self, value):
        self._freeze_order_id = value
    @property
    def industry(self):
        return self._industry

    @industry.setter
    def industry(self, value):
        self._industry = value
    @property
    def open_id(self):
        return self._open_id

    @open_id.setter
    def open_id(self, value):
        self._open_id = value
    @property
    def user_id(self):
        return self._user_id

    @user_id.setter
    def user_id(self, value):
        self._user_id = value


    def to_alipay_dict(self):
        params = dict()
        if self.benefit_use_infos:
            if isinstance(self.benefit_use_infos, list):
                for i in range(0, len(self.benefit_use_infos)):
                    element = self.benefit_use_infos[i]
                    if hasattr(element, 'to_alipay_dict'):
                        self.benefit_use_infos[i] = element.to_alipay_dict()
            if hasattr(self.benefit_use_infos, 'to_alipay_dict'):
                params['benefit_use_infos'] = self.benefit_use_infos.to_alipay_dict()
            else:
                params['benefit_use_infos'] = self.benefit_use_infos
        if self.biz_date:
            if hasattr(self.biz_date, 'to_alipay_dict'):
                params['biz_date'] = self.biz_date.to_alipay_dict()
            else:
                params['biz_date'] = self.biz_date
        if self.biz_no:
            if hasattr(self.biz_no, 'to_alipay_dict'):
                params['biz_no'] = self.biz_no.to_alipay_dict()
            else:
                params['biz_no'] = self.biz_no
        if self.freeze_order_id:
            if hasattr(self.freeze_order_id, 'to_alipay_dict'):
                params['freeze_order_id'] = self.freeze_order_id.to_alipay_dict()
            else:
                params['freeze_order_id'] = self.freeze_order_id
        if self.industry:
            if hasattr(self.industry, 'to_alipay_dict'):
                params['industry'] = self.industry.to_alipay_dict()
            else:
                params['industry'] = self.industry
        if self.open_id:
            if hasattr(self.open_id, 'to_alipay_dict'):
                params['open_id'] = self.open_id.to_alipay_dict()
            else:
                params['open_id'] = self.open_id
        if self.user_id:
            if hasattr(self.user_id, 'to_alipay_dict'):
                params['user_id'] = self.user_id.to_alipay_dict()
            else:
                params['user_id'] = self.user_id
        return params

    @staticmethod
    def from_alipay_dict(d):
        if not d:
            return None
        o = AlipayVoyagerMarketingRedeemModel()
        if 'benefit_use_infos' in d:
            o.benefit_use_infos = d['benefit_use_infos']
        if 'biz_date' in d:
            o.biz_date = d['biz_date']
        if 'biz_no' in d:
            o.biz_no = d['biz_no']
        if 'freeze_order_id' in d:
            o.freeze_order_id = d['freeze_order_id']
        if 'industry' in d:
            o.industry = d['industry']
        if 'open_id' in d:
            o.open_id = d['open_id']
        if 'user_id' in d:
            o.user_id = d['user_id']
        return o


