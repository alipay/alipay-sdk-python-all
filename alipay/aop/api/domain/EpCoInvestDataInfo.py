...
from alipay.aop.api.domain.EpCoInvestInfo import EpCoInvestInfo


class EpCoInvestDataInfo(object):
    ...
    def hits(self, value):
        if isinstance(value, list):
            self._hits = list()
            for i in value:
                if isinstance(i, EpCoInvestInfo):
                    self._hits.append(i)
                else:
                    self._hits.append(EpCoInvestInfo.from_alipay_dict(i))
    @property
    def total(self):
        return self._total
