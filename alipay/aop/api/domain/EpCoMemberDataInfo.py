...
from alipay.aop.api.domain.EpCoMemberInfo import EpCoMemberInfo


class EpCoMemberDataInfo(object):
    ...
    def hits(self, value):
        if isinstance(value, list):
            self._hits = list()
            for i in value:
                if isinstance(i, EpCoMemberInfo):
                    self._hits.append(i)
                else:
                    self._hits.append(EpCoMemberInfo.from_alipay_dict(i))
    @property
    def total(self):
        return self._total
