...
from alipay.aop.api.domain.LineStationRealTimeVO import LineStationRealTimeVO


class AlipayCommerceTransportMetroRealtimeQueryResponse(AlipayResponse):

    def __init__(self):
        super(AlipayCommerceTransportMetroRealtimeQueryResponse, self).__init__()
        self._lbs = None
        self._lines = None
        self._link = None
    ...
    def lines(self, value):
        if isinstance(value, list):
            self._lines = list()
            for i in value:
                if isinstance(i, LineStationRealTimeVO):
                    self._lines.append(i)
                else:
                    self._lines.append(LineStationRealTimeVO.from_alipay_dict(i))
    @property
    def link(self):
        return self._link
    ...
    def parse_response_content(self, response_content):
        response = super(AlipayCommerceTransportMetroRealtimeQueryResponse, self).parse_response_content(response_content)
        if 'lbs' in response:
