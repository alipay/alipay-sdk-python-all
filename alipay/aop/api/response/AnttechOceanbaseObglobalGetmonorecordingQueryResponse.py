...
from alipay.aop.api.domain.MonoRecordDTO import MonoRecordDTO


class AnttechOceanbaseObglobalGetmonorecordingQueryResponse(AlipayResponse):

    def __init__(self):
        super(AnttechOceanbaseObglobalGetmonorecordingQueryResponse, self).__init__()
    ...
    def result(self, value):
        if isinstance(value, MonoRecordDTO):
            self._result = value
        else:
            self._result = MonoRecordDTO.from_alipay_dict(value)

    def parse_response_content(self, response_content):
        response = super(AnttechOceanbaseObglobalGetmonorecordingQueryResponse, self).parse_response_content(response_content)
