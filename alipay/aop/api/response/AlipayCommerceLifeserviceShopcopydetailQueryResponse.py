...
from alipay.aop.api.domain.LsqBusinessLicenseInfo import LsqBusinessLicenseInfo
from alipay.aop.api.domain.LsqContactWayInfo import LsqContactWayInfo
from alipay.aop.api.domain.LsqLegalPersonInfo import LsqLegalPersonInfo
from alipay.aop.api.domain.LsqStoreAddressInfo import LsqStoreAddressInfo


class AlipayCommerceLifeserviceShopcopydetailQueryResponse(AlipayResponse):

    def __init__(self):
        super(AlipayCommerceLifeserviceShopcopydetailQueryResponse, self).__init__()
    ...
    def business_license_info(self, value):
        if isinstance(value, LsqBusinessLicenseInfo):
            self._business_license_info = value
        else:
            self._business_license_info = LsqBusinessLicenseInfo.from_alipay_dict(value)
    @property
    ...
    def contact_ways(self, value):
        if isinstance(value, list):
            ...
            for i in value:
                if isinstance(i, LsqContactWayInfo):
                    self._contact_ways.append(i)
                else:
                    self._contact_ways.append(LsqContactWayInfo.from_alipay_dict(i))
    @property
    def copy_id(self):
    ...
    def legal_person_info(self, value):
        if isinstance(value, LsqLegalPersonInfo):
            self._legal_person_info = value
        else:
            self._legal_person_info = LsqLegalPersonInfo.from_alipay_dict(value)
    @property
    def mcc_code(self):
    ...
    def store_address_info(self, value):
        if isinstance(value, LsqStoreAddressInfo):
            self._store_address_info = value
        else:
            self._store_address_info = LsqStoreAddressInfo.from_alipay_dict(value)
    @property
    def store_id(self):
    ...
    def parse_response_content(self, response_content):
        response = super(AlipayCommerceLifeserviceShopcopydetailQueryResponse, self).parse_response_content(response_content)
