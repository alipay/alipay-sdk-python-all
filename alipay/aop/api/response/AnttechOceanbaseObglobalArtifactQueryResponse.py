...
from alipay.aop.api.domain.ArtifactProjectNameDTO import ArtifactProjectNameDTO


class AnttechOceanbaseObglobalArtifactQueryResponse(AlipayResponse):

    def __init__(self):
        super(AnttechOceanbaseObglobalArtifactQueryResponse, self).__init__()
    ...
    def result(self, value):
        if isinstance(value, ArtifactProjectNameDTO):
            self._result = value
        else:
            self._result = ArtifactProjectNameDTO.from_alipay_dict(value)

    def parse_response_content(self, response_content):
        response = super(AnttechOceanbaseObglobalArtifactQueryResponse, self).parse_response_content(response_content)
