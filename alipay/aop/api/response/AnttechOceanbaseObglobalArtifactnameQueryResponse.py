...
from alipay.aop.api.domain.ArtifactNameDTO import ArtifactNameDTO


class AnttechOceanbaseObglobalArtifactnameQueryResponse(AlipayResponse):

    def __init__(self):
        super(AnttechOceanbaseObglobalArtifactnameQueryResponse, self).__init__()
    ...
    def result(self, value):
        if isinstance(value, ArtifactNameDTO):
            self._result = value
        else:
            self._result = ArtifactNameDTO.from_alipay_dict(value)

    def parse_response_content(self, response_content):
        response = super(AnttechOceanbaseObglobalArtifactnameQueryResponse, self).parse_response_content(response_content)
